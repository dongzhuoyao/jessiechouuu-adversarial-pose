...
local function parse(arg)
    ...
    cmd:option('-dataset',        'LSP', 'Dataset choice: mpii | flic')
    ...
    cmd:option('-netD',           'none', 'Provide full path to a previously trained netD')
    ...
    cmd:text()
    cmd:option('-lastEpoch',            'none', 'How often to take a snapshot of the model (0 = never)')
    cmd:option('-snapshot',            6, 'How often to take a snapshot of the model (0 = never)')
    ...
    cmd:option('-LR_D',             8e-5, 'Learning rate of discriminator')
    cmd:option('-LRdecay_D',         0.0, 'Learning rate decay of discriminator')
    cmd:option('-momentum_D',        0.0, 'Momentum of discriminator')
    cmd:option('-weightDecay_D',     0.0, 'Weight decay of discriminator')
    cmd:option('-alpha_D',          0.99, 'Alpha of discriminator')
    cmd:option('-epsilon_D',        1e-8, 'Epsilon of discriminator')
    cmd:text()
    ...
    cmd:option('-validIters',          0, 'Number of validation iterations per epoch')
    ...
    cmd:text()
    cmd:text(' ---------- Adversarial options --------------------------------')
    cmd:text()
    cmd:option('-lambda_G',       0.0001, 'Weight of G1 and G2')
    cmd:option('-lambda_k',        0.001, 'Weight to clip k_t')
    cmd:option('-gamma',             0.5, 'Balance weight of real and fake')
    cmd:option('-init_Kt',           0.0, 'Balance weight of real and fake')
