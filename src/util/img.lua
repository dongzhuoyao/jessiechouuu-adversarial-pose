...
function drawSkeleton(img, preds, idx,scores)
    ...
    image.save(paths.concat(projectDir,'prediction/'..idx..'.jpg'),img)
    return img
end
...
function heatmapVisualization(set, idx, pred, inp, gt)
    ...
    tmpInp = image.scale(tmpInp,res):mul(.2)
    --tmpInp[1][1][1] = 1
    ...
    if pred then -- Copy predicted heatmaps to blue channel
        hmImg:sub(1,-1,1,1):add(pred:clone():mul(.9))
    end
    
    local max, idx = torch.max(pred:view(pred:size(1), pred:size(2), pred:size(3) * pred:size(4)), 3)
    local preds = torch.repeatTensor(idx, 1, 1, 2):float()
    preds[{{}, {}, 1}]:apply(function(x) return (x - 1) % pred:size(4) + 1 end)
    preds[{{}, {}, 2}]:add(-1):div(pred:size(3)):floor():add(1)
    
    for i=1,16 do
      hmImg[i] = image.drawRect(hmImg[i], preds[1][i][1], preds[1][i][2], preds[1][i][1]+1, preds[1][i][2]+1, {lineWidth = 1, color = {0, 0, 255}})
    end
