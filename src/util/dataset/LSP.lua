...
function Dataset:__init()
    ...
    local a = hdf5.open(paths.concat(projectDir,'data/lsp_mpii.h5'),'r')
...
function Dataset:getPath(idx)
    ...
    return paths.concat(projectDir, 'data', filename)
end
...
function Dataset:getPartInfo(idx)
    ...
    local dataset = string.split(ffi.string(self.annot.imgname[idx]:char():data()), '/')[1]
